...
def parse_route_csv(filename):
    """
    Parses a route CSV file and extracts x, y coordinates.
    Expected format is a CSV where the first two columns are x and y.
    """
    try:
        data = np.genfromtxt(filename, delimiter=',', skip_header=1)
        x = data[:, 0]
        ...
        print(f"Error parsing route file {filename}: {e}")
...
def add_safety_colorbar(figure):
    ...
    sm = mpl_cm.ScalarMappable(cmap=CMAP)
    cbar = figure.colorbar(sm, cax=cbar_ax)
    cbar.set_label("Comfort/Safety Level")

def plot_route(axis, route_x, route_y):
    """Plots the route specified in the CSV file"""
    if len(route_x) > 0:
        axis.plot(route_y, route_x, color='green', linewidth=2, label='Route')
        axis.scatter(route_y[0], route_x[0], color='green', marker='o', s=100, label='Start')
        axis.scatter(route_y[-1], route_x[-1], color='red', marker='x', s=100, label='End')
    
    axis.set_xlabel("Y Position (m)")
    axis.set_ylabel("X Position (m)")
    axis.set_title("Planned Route")
    axis.legend()
    axis.grid(True)

def plot_metrics(time_jerk, jerk, time_heading_acc, heading_acc, vehicle_time, cte, 
                 x_actual, y_actual, x_desired, y_desired, pedestrian_times, pedestrian_distances, route_x=None, route_y=None):
    ...
    # Plot route in the bottom right
    if route_x is not None and route_y is not None:
        plot_route(axs[1,2], route_x, route_y)
    else:
        axs[1,2].axis('off')
...
if __name__=='__main__':
    if len(sys.argv) < 2:
        print("Usage: python test_comfort_metrics.py <log_directory> [route_file]")
        sys.exit(1)
    ...
    # Try to get route file from command line or look for default one
    route_x, route_y = None, None
    if len(sys.argv) > 2:
        route_file = sys.argv[2]
        if os.path.exists(route_file):
            route_x, route_y = parse_route_csv(route_file)
    else:
        # Try to find default route file from GEMstack directory
        default_route = os.path.join(os.getcwd(), "GEMstack/knowledge/routes/xyhead_highbay_backlot_p.csv")
        if os.path.exists(default_route):
            route_x, route_y = parse_route_csv(default_route)
            print(f"Using default route file: {default_route}")
    ...
    if os.path.exists(tracker_file):
        ...
        plot_metrics(time_jerk, jerk, time_heading_acc, heading_acc, vehicle_time, cte, 
                     x_actual, y_actual, x_desired, y_desired, ped_times, ped_distances, route_x, route_y)
    ...
    # Pure pursuit tracker file is missing: plot only behavior.json metrics
        ...
        # Plot only jerk, heading acceleration, pedestrian distance, and route if available
        if route_x is not None and route_y is not None:
            ...
            plot_route(axs[3], route_x, route_y)
        else:
